  ...
  return (
    <div style={{ background: '#F0F4F0', minHeight: '100vh', paddingBottom: '80px' }}>
      ...
      <div style={{ background: '#FFFFFF', borderBottom: '1px solid #D8E4D8', padding: '20px 16px 12px' }}>
        ...
        <h1 style={{ fontFamily: 'Lora', fontSize: '22px', color: '#3D2B1F', margin: '0 0 4px' }}>Books Read</h1>
      ...
      <div style={{ background: '#FFFFFF', padding: '12px 16px', borderBottom: '1px solid #D8E4D8' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
          ...
          <span style={{ fontFamily: 'Outfit', fontSize: '12px', color: '#2E7D52', fontWeight: 600 }}>{Math.round((books.length / 40) * 100)}%</span>
        </div>
        <div style={{ background: '#D8E4D8', borderRadius: '4px', height: '6px' }}>
          <div style={{ background: '#2E7D52', borderRadius: '4px', height: '6px', width: `${Math.min((books.length / 40) * 100, 100)}%`, transition: 'width 0.3s' }} />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px 24px' }}>
          ...
          <p style={{ fontFamily: 'Lora', fontSize: '18px', color: '#3D2B1F', margin: '0 0 8px' }}>No books yet</p>
          <p style={{ fontFamily: 'Outfit', fontSize: '14px', color: '#7A8F7A', margin: 0 }}>Tap "Add Book" when you finish one!</p>
        </div>
      ) : (
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {books.map((book, i) => (
            <div key={book.id} style={{ background: '#FFFFFF', borderRadius: '12px', padding: '14px 16px', border: '1px solid #D8E4D8' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                    <span style={{ fontFamily: 'Lora', fontSize: '15px', color: '#3D2B1F', fontWeight: 600 }}>#{i + 1} {book.title}</span>
                  ...
                  {book.author && <p style={{ fontFamily: 'Outfit', fontSize: '13px', color: '#7A8F7A', margin: '0 0 4px' }}>by {book.author}</p>}
                  <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                    {book.genre && <span style={{ fontFamily: 'Outfit', fontSize: '11px', color: '#7A9E7E', background: '#F7F9F7', padding: '2px 8px', borderRadius: '4px' }}>{book.genre}</span>}
                    {book.rating > 0 && <span style={{ fontSize: '12px' }}>{'⭐'.repeat(book.rating)}</span>}
                    <span style={{ fontFamily: 'Outfit', fontSize: '11px', color: '#7A8F7A' }}>{book.finished_date}</span>
                  </div>
                </div>
                <button onClick={() => handleDelete(book.id)} style={{ background: 'none', border: 'none', color: '#7A8F7A', cursor: 'pointer', fontSize: '18px', padding: '0 0 0 8px' }}>×</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div style={{ background: '#FFFFFF', borderRadius: '20px 20px 0 0', width: '100%', maxWidth: '480px', maxHeight: '85vh', overflow: 'auto' }}>
            ...
            <div style={{ padding: '20px 20px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h2 style={{ fontFamily: 'Lora', fontSize: '18px', color: '#3D2B1F', margin: 0 }}>Add a Book</h2>
            ...
            <div style={{ padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              <div>
                <label style={{ fontFamily: 'Outfit', fontSize: '12px', color: '#7A8F7A', display: 'block', marginBottom: '4px' }}>AUTHOR</label>
              ...
              <div>
                <label style={{ fontFamily: 'Outfit', fontSize: '12px', color: '#7A8F7A', display: 'block', marginBottom: '4px' }}>GENRE</label>
                <select value={form.genre} onChange={e => setForm({ ...form, genre: e.target.value })}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #D8E4D8', background: '#F7F9F7', fontFamily: 'Outfit', fontSize: '14px', color: '#1C2B1C' }}>
              ...
              <div>
                <label style={{ fontFamily: 'Outfit', fontSize: '12px', color: '#7A8F7A', display: 'block', marginBottom: '8px' }}>RATING</label>
              ...
              <div>
                <label style={{ fontFamily: 'Outfit', fontSize: '12px', color: '#7A8F7A', display: 'block', marginBottom: '4px' }}>FINISHED DATE</label>
                <input type="date" value={form.finished_date} onChange={e => setForm({ ...form, finished_date: e.target.value })}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #D8E4D8', background: '#F7F9F7', fontFamily: 'Outfit', fontSize: '14px', color: '#1C2B1C', boxSizing: 'border-box' }} />
              </div>

              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#F7F9F7', padding: '12px', borderRadius: '8px' }}>
                <div>
                  <p style={{ fontFamily: 'Outfit', fontSize: '14px', color: '#1C2B1C', margin: '0 0 2px', fontWeight: 500 }}>Tamil book?</p>
                  <p style={{ fontFamily: 'Outfit', fontSize: '12px', color: '#7A8F7A', margin: 0 }}>Counts toward your 4 Tamil goal</p>
                </div>
                <div onClick={() => setForm({ ...form, is_tamil: !form.is_tamil })}
                  style={{ width: '44px', height: '24px', borderRadius: '12px', background: form.is_tamil ? '#2E7D52' : '#D8E4D8', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' }}>
